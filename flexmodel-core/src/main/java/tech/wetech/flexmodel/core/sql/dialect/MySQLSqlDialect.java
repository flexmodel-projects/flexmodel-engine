...
//        registerColumnType( Types.TIMESTAMP, "datetime(6)" );
  ...
  protected final SequenceExporter sequenceExporter = new MySQLSequenceExporter(this);
...
//		registerColumnType( Types.VARCHAR, 16777215, "mediumtext" );
  ...
  public long getSequenceNextVal(String sequenceName, SqlExecutor sqlExecutor) {
    ...
    sqlExecutor.updateAndReturnGeneratedKeys(
      "update " + quoteIdentifier(sequenceName) + " set nextval=last_insert_id(nextval + incr)",
      new String[]{"nextval"}, keys -> key.set((long) keys.get(0))
    );
    return key.get();
  }
