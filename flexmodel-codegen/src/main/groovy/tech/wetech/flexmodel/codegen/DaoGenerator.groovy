...
class DaoGenerator extends AbstractGenerator {
  ...
  def generate(PrintWriter out, GenerationContext context) {
    ...
    if (modelClass.idField) {
      ...
      out.println "      // Convert record to a map and update the record with the given ID"
      out.println "      Map map = jsonObjectConverter.convertValue(record, Map.class);"
      out.println "      ((Set<Map.Entry>) map.entrySet()).removeIf(e -> e.getValue() == null);"
      out.println "      return session.updateById(modelName, map, id);"
      out.println "    }"
      out.println "  }"
      out.println ""
    ...
    out.println "   * @return The number of rows affected by the update operation."
    out.println "   */"
    out.println "  public <S extends ${modelClass.shortClassName}> int update(S record, UnaryOperator<Query> unaryOperator) {"
    out.println "    try (Session session = sessionFactory.createSession(schemaName)) {"
    ...
    out.println "   * @return The number of rows affected by the update operation."
    out.println "   */"
    out.println "  public <S extends ${modelClass.shortClassName}> int updateIgnoreNull(S record, UnaryOperator<Query> unaryOperator) {"
    out.println "    try (Session session = sessionFactory.createSession(schemaName)) {"
    out.println "      // Convert record to a map and update records matching the criteria"
    out.println "      Map map = jsonObjectConverter.convertValue(record, Map.class);"
    out.println "      ((Set<Map.Entry>) map.entrySet()).removeIf(e -> e.getValue() == null);"
    ...
    out.println "   */"
    out.println "  public int delete(UnaryOperator<Query> unaryOperator) {"
    out.println "    try (Session session = sessionFactory.createSession(schemaName)) {"
