...
class DaoGenerator extends AbstractGenerator {
  ...
  def generate(GroovyPrintWriter out, className, GenerationContext context) {
    ...
    out.println "    if (record.get${modelClass.idField.shortTypeName.capitalize()}() != null) {"
    out.println "      updateById(record, record.get${modelClass.idField.shortTypeName.capitalize()}());"
    out.println "    } else {"
    out.println "      create(record, id -> record.set${modelClass.idField.shortTypeName.capitalize()}(id));"
    out.println "    }"
