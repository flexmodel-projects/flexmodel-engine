...
import tech.wetech.flexmodel.test.dto.TeacherDTO;
import tech.wetech.flexmodel.test.entity.Classes;
import tech.wetech.flexmodel.test.entity.Student;
import tech.wetech.flexmodel.test.entity.StudentDetail;
...
import static tech.wetech.flexmodel.core.model.field.GeneratedValue.AUTO_INCREMENT;
import static tech.wetech.flexmodel.core.model.field.GeneratedValue.UUID;
import static tech.wetech.flexmodel.core.query.Direction.DESC;
...
public abstract class AbstractSessionTests {
  ...
  private void testOneToOneRelation(String studentEntityName, String studentDetailEntityName) {
    ...
    Map<String, Object> oneToOne = session.find(studentEntityName, query).getFirst();
  ...
  private void testOneToManyRelation(String classesEntityName, String studentEntityName) {
    ...
    List<Map<String, Object>> oneToMany = session.find(classesEntityName, query);
  ...
  void testCountByCondition() {
    ...
    long total = session.count(entityName, query);
  ...
  void testExistsByCondition() {
    ...
    createTeacherCollection2(entityName);
    boolean exists = session.exists(entityName, query -> query.where("""
      {
        "_or": [
          {
            "name": {
  ...
  void testFindAll() {
    ...
    createTeacherCollection2(entityName);
    List<Map<String, Object>> list = session.find(entityName, createSimpleQuery());
    Assertions.assertFalse(list.isEmpty());
  }
  ...
  void testFindAllWhenResultDto() {
    ...
    createTeacherCollection2(entityName);
    List<TeacherDTO> list = session.find(entityName, createSimpleQuery(), TeacherDTO.class);
    Assertions.assertFalse(list.isEmpty());
  ...
  void testDropField() {
    ...
    createStudentCollection2(entityName);
    session.createIndex(new IndexDefinition(entityName, "IDX_name").addField("name"));
    session.createIndex(
